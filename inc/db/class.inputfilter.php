...
class InputFilter {
		...
		if (is_array($source)) 
		{
			...
			foreach($source as $key => $value)
			{
				...
				if (is_string($value)) 
				{
					if(is_numeric($value))
					{
						$source2[$key] = $source[$key];
					}
					else
					{
						$something = $this->decode($value);
						$source2[$key] = $this->remove($something);
					}
				}
				else 
				{
	...
	{
		// url decode
		$source2 = html_entity_decode($source, ENT_QUOTES, "ISO-8859-1");
		// convert decimal
		$source3 = preg_replace('/&#(\d+);/me',"chr(\\1)", $source2);				// decimal notation
		// convert hex
		$source4 = preg_replace('/&#x([a-f0-9]+);/mei',"chr(0x\\1)", $source);	// hex notation
		return $source;
	}
		...
		if (is_array($source)) 
		{
			...
			foreach($source as $key => $value)
			{
				...
				if (is_string($value)) 
				{
					...
					{
						$source2[$key] = $this->quoteSmart($this->decode($value), $connection);
					}
				}
				else
				{
	...
	function escapeString($string, &$connection) {
		...
		if (version_compare(phpversion(),"4.3.0", "<")) DB::escape($string);
		// current function
		else DB::escape($string);
		return $string;
	}
}
?>